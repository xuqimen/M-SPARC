function [S] = scf_sq(S)
% @brief    SCF_SQ(S) implements the self consistent field calculation.
%
...
while (err > S.SCF_tol && count_SCF <= max_scf_iter || count_SCF <= min_scf_iter)
	...
	%S.densMatFlag = 1; % TODO: make this an input option
	if S.densMatFlag == 0
	...
	if (((S.ForceCount == 1) && (count >= max_count_first_relax)) ...
		...
		S.scf_err = err; % update scf_err in S
% 		if S.scf_err < 1e-4
